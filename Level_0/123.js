...
const sol = {
    "0": (arr) => arr,
    "1": (arr, dif) => arr.map(elem => [...elem, ...new Array(dif).fill(0)]),
    "2": (arr, dif) => [...arr, ...new Array(dif).fill(0).map(_ => new Array(arr[0].length).fill(0))],
}

const solution = (arr) => arr.length === arr[0].length ? sol[0](arr) : arr.length > arr[0].length ? sol[1](arr, arr.length - arr[0].length) : sol[2](arr, arr[0].length - arr.length);
