 ...
 * 등수가 높은 순서대로 각각 a, b, c번이라고 할 때 10000 × a + 100 × b + c를 return 하는 solution 함수를 작성해 주세요.
 */

const solution = (rank, attendance) => {
    const [a, b, c] = rank.reduce((acc, cur, idx) => (acc[cur] = attendance[idx] ? idx : 999, acc), []).filter(v => v !== 999);
